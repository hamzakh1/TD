...
#pragma endregion
gsl::span<Concepteur*> spanListeDesigners(const Liste<Jeu>& liste)
{
	return gsl::span(liste.getElement(), liste.getNElement());
}
shared_ptr<Concepteur> chercherConcepteur(Liste<Jeu>& listeJeux, std::string nom)
{
	//span<Jeu*> spanJeux(listeJeux.elements_, size_t(listeJeux.nElements_));
