...
Designer* chercherDesigner(const string& nom, ListeJeux listeJeux ) //const
{//a revoir !!!
	span<Jeu*> spanJeu(listeJeux.elements, size_t(listeJeux.nElements));
	for (Jeu* jeu : spanJeu)
	{
		span<Designer*> spanDesigner(jeu->designers.elements, size_t(jeu->designers.nElements));
		for (Designer* designer : spanDesigner)
		{
			if (designer->nom == nom)
			{
				return designer;
...
Designer* lireDesigner(istream& fichier, ListeJeux jeu)
{
	...
	Designer* verification = chercherDesigner(designer.nom, jeu );
	...
	if (verification == nullptr)
	{
		...
		return verification; //TODO: Retourner le pointeur vers le designer crée.
}
...
//void ListeJeux::chercheDesigner(Designer* designer)  <- pas sur si marche, pourrait servir a enlever la repetition de ListeJeux
void ajouterJeu(ListeJeux& listeJeux, Jeu* jeu)
{// trouver facon d'enlever listeJeux
	if (listeJeux.capacite <= 0) // reinitialisation si la capacite est invalide
	{
	...
	{
		Jeu** nouveauTableau = new Jeu * [listeJeux.capacite * 2]; //tableau 2x plus grand
		for (int i : range(listeJeux.capacite))
			nouveauTableau[i] = listeJeux.elements[i];
		
		delete[] listeJeux.elements;
		listeJeux.elements = nouveauTableau;
	}
	listeJeux.elements[listeJeux.nElements++] = jeu; // a revoir
...
void enleverJeu(Jeu* jeu, ListeJeux& listeJeux)
{
	...
	for (int i = 0; i < listeJeux.nElements;++i)
	{
		...
		{
			listeJeux.elements[i - 1] = listeJeux.elements[--listeJeux.nElements];
			listeJeux.elements[--listeJeux.nElements] = nullptr;
			listeJeux.nElements--;
